...
name_image_output = 'Test_plot_06_1'
...
diff_data_mon = 37 # the timing of the monitoring data is 27 sec ahead of the pi in the pressure vessel
...
t_power_ON = 5205 # In sec, when the power was turned ON
...
#visualisation limit
visual_x_lim_low = None
visual_x_lim_high = None

## Retrieve data
data_path = os.path.join(file_path, 'test_bang_bang_adjust_2020-09-11_16-32-31_TEST_06_1_data.csv')
...
monitoring_data = os.path.join(file_path, 'monitoring_vacuum_chamber_2020-09-17_02-33-19_TEST_06_1_data.csv')
t, R1_1, R1_2, R1_3, R1_4, R2_1, R2_2, R2_3, R2_4, R3_1, ADC_1_temp, ADC_2_temp, ADC_3_temp, pressure, power_voltage, PWM_disc, PWM_fluid,pw_disc, pw_fluid, pw_total, ex_low_temp_disc, low_temp_disc, high_temp_disc, ex_high_temp_disc, ex_low_PWM_disc, low_PWM_disc, high_PWM_disc, ex_high_PWM_disc, ex_low_temp_fluid, low_temp_fluid, high_temp_fluid, ex_high_temp_fluid, ex_low_PWM_fluid, low_PWM_fluid, high_PWM_fluid, ex_high_PWM_fluid = np.genfromtxt(data_path,delimiter=',',skip_header=1,unpack=True)
tm, volt_sensor, pressure_chamber, R1_1_out, R1_2_out, temp_ADS = np.genfromtxt(monitoring_data,delimiter=',',skip_header=1,unpack=True)
...
### Data process
## Put the PWM and Power calculation at 0 until t = 5205 - 37 sec, since the power was disconnected
i=0
while t[i] < t_power_ON - diff_data_mon:
        i=i+1
index_power_on = i
for i in range(0,index_power_on):
    PWM_disc[i] = 0;PWM_fluid[i]=0;pw_disc[i]=0;pw_fluid[i]=0;pw_total[i]=0
...
if t_offset_data > 0:
    ...
    R3_1=np.delete(R3_1,index_offset_data); ADC_1_temp=np.delete(ADC_1_temp,index_offset_data); ADC_2_temp=np.delete(ADC_2_temp,index_offset_data); ADC_3_temp=np.delete(ADC_3_temp,index_offset_data)
...
if t_offset_monitor_data > 0:
    ...
    R1_1_out = np.delete(R1_1_out,index_offset_monitor_data); R1_2_out= np.delete(R1_2_out,index_offset_monitor_data); temp_ADS= np.delete(temp_ADS,index_offset_monitor_data)
    t_off  = tm[0]
...
def filter_aberations(temp_list, threshold, nb_point_to_average):
    initial_list_lenght = len(temp_list) # will be use to check that the modified list has the right length
...
if visual_x_lim_low != None:
    ...
    ##get index of of the item value visual_x_lim_low on the pressure vessel data set
    i = 0
...
#plt.plot(t, R1_2, color='tab:blue', marker = "+", linewidth=1, label = 'RTD 1_2, surface of the disc')
plt.plot(t, R1_3, color='tab:orange', marker = "+", linewidth=1, label = 'RTD 1_3, surface of the disc, used to regulate the disc heater')
plt.plot(t, R1_4, color='tab:green', marker = "+", linewidth=1, label = 'RTD 1_4, inside a disc chamber')
plt.plot(t, R2_1, color='chartreuse',  marker = "+", linewidth=1, label = 'RTD 2_1, inside a disc chamber')
plt.plot(t, R2_4, color='tab:olive', marker = "+", linewidth=1, label = "RTD 2_4, inside a disc chamber")
...
plt.title('Temperature on different points of MCSD,\ninside chamber and on the surface to control the heater\nduring the pseudo thermal vacuum preliminary test 6.1')  
plt.xlabel('Time (sec)', loc = 'right'); plt.ylabel('Temperature Celcius')
...
plt.plot(t, R1_1, color='salmon',  marker = "+", linewidth=1, label = "RTD 1_1, on the RPi")
plt.plot(t, R2_2, color='tab:pink', marker = "+", linewidth=1, label = "RTD 2_2, on the fluidic bag, regulating the fluidic heater")
plt.plot(t, R2_3, color='tab:grey', marker = "+", linewidth=1, label = "RTD 2_3, at the end of the fluidic's bag tube")
plt.plot(t, R3_1, color='blue', dashes=[6, 2], linewidth=1, label = "RTD 3_1, air at the electronic side")
...
# PRESSURE plot
plt.subplot(3,1,2)
plt.plot(t, pressure, color='black', label = "Pressure inside the pressure vessel (kPa)")

plt.title('pressure measurements inside the pressure vessel\nduring the pseudo thermal vacuum preliminary test 6.1')  
...
# POWER plot
plt.figure(figsize=(8.7,11), dpi=300) # Plot on a A4 format ==> easier implementation in text
plt.step(t, pw_disc, color='tab:red', where='post', marker = '.', linewidth=1, label = 'Power at disc heater. Average Power: '+ '%.2f' % averge_disc_power +' W. Total energy consumed: '+ '%.2f' % total_energy_disc + ' J.')
...
#plt.ylim(0, 12)
plt.twinx()
plt.plot(t, R1_3, color='salmon',  marker = "+", linewidth=1, label = "RTD 1_3, temperature on the disc surface regulating the disc")
plt.plot(t, ex_high_temp_disc, color='coral', dashes=[6, 2], linewidth=0.5, label =  'temperature extreme high threshold for disc heater regulation')
plt.plot(t, high_temp_disc, color='lightsalmon', dashes=[6, 2], linewidth=0.5, label = 'temperature high threshold for disc heater regulation')
plt.plot(t, low_temp_disc, color='salmon', dashes=[6, 2], linewidth=0.5, label = 'temperature low threshold for disc heater regulation')
plt.plot(t, ex_low_temp_disc, color='tomato', dashes=[6, 2], linewidth=0.5, label = 'temperature extreme low threshold for disc heater regulation')

plt.plot(t, R2_2, color='aqua',  marker = "+", linewidth=1, label = "RTD 2_2, temperature on the fluidic bag regulating the fluid")
plt.plot(t, ex_high_temp_fluid, color='darkcyan', dashes=[6, 2], linewidth=0.5, label =  'temperature extreme high threshold for fluid heater regulation')
plt.plot(t, high_temp_fluid, color='c', dashes=[6, 2], linewidth=0.5, label = 'temperature high threshold for fluid heater regulation')
plt.plot(t, low_temp_fluid, color='cyan', dashes=[6, 2], linewidth=0.5, label = 'temperature low threshold for fluid heater regulation')
plt.plot(t, ex_low_temp_fluid, color='dodgerblue', dashes=[6, 2], linewidth=0.5, label = 'temperature extreme low threshold for fluid heater regulation')

plt.ylabel('Temperature (°C) ')
plt.xlim(visual_x_lim_low, visual_x_lim_high)
#plt.ylim(0, 12)
plt.legend(loc='upper left', bbox_to_anchor=(-0.05, -0.3), ncol = 2, fontsize = 'small')
...
plt.plot(tm, R1_1_out, color='blue', marker = "+", linewidth=1, label = "RTD 1_1, the temperature on  on a 1U aluminium panel")
plt.plot(tm, R1_2_out, color='green', marker = "+",linewidth=1, label = "RTD 1_2, the temperature on raspberry pi processor")
plt.title('Temperatures measured outside the pressure vessel inside the vacuum chamber\nduring the pseudo thermal vacuum preliminary test 6')
plt.xlabel('Time (sec)', loc = 'right'); plt.ylabel('Temperature (Celsius)')
plt.grid(axis='x', color='0.95')
plt.xlim(visual_x_lim_low, visual_x_lim_high)
#plt.ylim(-10, 10)
...
# PRESSURE plot
plt.subplot(3,1,3)
plt.plot(tm, pressure_chamber, color='black', linewidth=1, label = "Pressure of the vacuum chamber")
