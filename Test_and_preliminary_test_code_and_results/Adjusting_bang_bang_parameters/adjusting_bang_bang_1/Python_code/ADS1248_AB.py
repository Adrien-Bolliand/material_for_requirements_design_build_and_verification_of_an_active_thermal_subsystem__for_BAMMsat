 ...
 #  The deaulkt configuration is made to Read RTD sensor in a ratiometric measurment
 ...
 #  Adapted for BAMMsat on BEXUS (BoB) by Adrien Bolliand
    # Changed the order of the methodes
    ...
    #   - function to read RTDs (a Ratiometric measurment is perfomred)
    #   - function to read 5V input sensor like the photoresitor, photodiode or the bubble sensor
 ...
 # ======================================================================================================================
 #
 # The attributs of the ADS1248 class are:
    # list          (list of the ADC instanciatied)
    # Rref          (float) Reference resistor for RTD ratiometric reading in ohm
    # Rmis          (list for 4 float) value of the RTD wires resistance mismatch. i = 0 ==> mismatch of RTD1; i = 1 ==> mismatch of RTD2; i = 2 ==> mismatch of RTD3; i = 3 ==> mismatch of RTD4
    # Vref_2        (flaot) Refer ence voltage for 5 V sonsor reading in volt
    # internal_Vref (float) teh internal reference of the ADS1248 nominally at 2.048 V
    # verbose       (bolean) will print status info if True
    ...
    # spi           (spi object) define the spi connnection (see example code)
    # freq          (int) data rate, MAX: 2 049 180 Hz  ~ 2 MHz
    # drdy          (digital pin) input of the raspberry pi. connected to all the DOUT/DRDY of the ADS1248 pulled low by the ADS1248 when data is ready to be transmited
 #
 # The methods of the ADS1248 class are:
    # setup (spi, drdy_pin, freq)                   Setup for all the ADS1248 of the BOB electronic. input the SPI object, the DRDY pin that will be use to monitor if the data is ready for all the ADS1248 and the frequence od the data rate
    # __init__(cs_pin, Rref,list_res_mis,Vref_2)    Object builder. input the Chip Select (CS) pin, the vvalue in ohm of the Reference resitor for RTD measuremnt, the list of the values in ohm of the resistance mismatch of the RTD wires form RTD calibration, the Reference voltage for other sensors  
    ...
    # selfOffset()                                  Start the self offset calibration procedure. Automaticaly write in the OFC1 and OFC2 (5 and 6) register the result
    # fetch(AINP+,AINN-)                            Reterive the 24 bits converted value between AINP+ and AINN-
    # receive ()                                    Used by the fecth methode to catch the 24 bits
    # Converstion_R_to_C(Resistance_of_an_RTD)      Using the CVM equation retun a temperature value in celcius form an RTD resistance value
    # Read_RTD(RTD_id_numnber)                      Read and return the resistance and the temperature of the RTD number "RTD_id_number". The wiring is specified below and in the method
 ...
 ## wiring RTDs
        ...
        #       RTD_id_numnber = 1 ==> AIN 0 (N) - AIN 1 (P)   (P) 1 wire side, positive voltage (N) 2 wire side, Negative voltage for the ADS1248 MUX
        #       RTD_id_numnber = 2 ==> AIN 2 (N) - AIN 3 (P)
        #       RTD_id_numnber = 3 ==> AIN 4 (N) - AIN 5 (P)
        #       RTD_id_numnber = 4 ==> AIN 6 (N) - AIN 7 (P)
        ##
 #
...
class ADS1248:
    ...
    internal_Vref = 2.068     # The internal reference voltage value of every ADS1248. Used to reaad the internal temperature of the chip
    # for the Callendar-Van Dusen equation (CVM equation)
    CVM_A  = 3.9083 * 10**-3  # °C^-1
    CVM_B  = -5.775 * 10**-7  # °C^-2
    CVM_Ro = 100              # ohm for Pt100 RTDs only

    def __init__(self, cs_pin, Rref = 820, list_res_mis = [0,0,0,0], Vref_2 = 2.5):
        ...
        self.Rmis = list_res_mis # dedfine the wire resistnace mismatche. value unique to each RTD and was obtained by calibraation
        self.vref_2 = Vref_2 # value of the second reference voltage, nominly use to read 5 V analog sensor. The regerence voltage should be around 2.5 V
        ...
        # send a reset command at initalisation
        self.rst()
        ...
        self.wreg(10,[0x0E]) # In the IDAC0 register (10)==> DOUT/DRDY mode 1 ENABLED, exitation current set to 1 mA
        self.wreg(3, [0x32]) # In the SYS0  register (3) ==> the PGA gains to 8 and Data rate to 20 SPS (0x32)
        self.G  = 8          # PGA gain
    ...
    def setup(pi, spi_handle, drdy_pin, freq=2000000, verbose = False):
        ...
        ADS1248.freq = freq # strore the data rate as an attribut
    ...
    ##
    ## generic methodes/commands for ADS in the BoB fligth configuration
    ##
    ...
    def wreg(self,register_int,data_hex_list): # the register has to be the number of the register between 0 and 15. data is a list of command in hexadecimal
        ADS1248.pigpio.write(self.cs, 0) # allow communication with the ADS1248
        time.sleep(1*10**(-8)) # t_CSSC wait time after CS is set to low before communication
        send = [64+register_int, len(data_hex_list)-1] + data_hex_list # generate the sequence to send by concatanation, "data" as to be a list
    ...
    def receive(self):
        ADS1248.pigpio.write(self.cs, 0) # allow communication with the ADS1248
        if ADS1248.pigpio.read(self.drdy): # if drdy pin is high, it mean that the daya will be ready. when DRDY pin goes low, it means that the signal has been converted into bytes and is ready to be read
            if self.verbose:
 ...
 #####
 ##### function for BoB set up
 #####

    def Converstion_R_to_C(self, Resistance_RTD):
        # using quadratic CVD equation working above 0 °C. Quadratic equation is prabably overkill for the experiment temperature range. linear would work
    ...
    def Read_RTD(self, RTD):
        ...
        current_source_1 = [Ain_n * 16 + Ain_p] # the data that shall be written into the register to set the curent source in the 1st configuration for the 1st reading to average
        current_source_2 = [Ain_p * 16 + Ain_n] # the data that shall be written into the register to set the curent source in the 2nd configuration for the 2nd reading to average
        self.wreg(11, current_source_1) # In the IDAC1 register (11) ==> set up the extitation current into the right analog input (AIN).
        raw = self.fetch(Ain_n,[Ain_p]) # retreive the conversion data of the 1st reading to average
        try: # if retreive is succesful convert the reading into the value of the RDT resistance in ohm
            reading_source_1 = self.Rmis[RTD-1] + 2*((raw[0]*self.rref)/(self.G*((2**23)-1))) # convert the 1st reading to average in ohm
            reading = True
        except:
            reading = False
        self.wreg(11,current_source_2) # swipe the current sources. In IDAC1 register (11) ==> set up the extitation current into AIN 0 and 1. The current that goes through the RTD is from the source 1
        raw = self.fetch(Ain_n,[Ain_p]) # retreive the conversion data of the 2nd reading to average
        try: # if retreive is succesfull convert the readind into the value of the RDT resistance in ohm
            reading_source_2 = self.Rmis[RTD-1] + 2*((raw[0]*self.rref)/(self.G*((2**23)-1))) # convert the 2nd reading to average in ohm
        except:
            reading = False
        self.wreg(11,[0xFF]) # In the IDAC1 register (11) ==> Shut OFF the extitation currents
        if reading: # if both reading were succesfull
            RTD_res = (reading_source_1 + reading_source_2)/2 # average the 2 reading in ohm to compensate for current source mismatch
            RTD_degree = self.Converstion_R_to_C(RTD_res) # convert averagedd ohm reading into temperature in °C
    ...
    def Read_5V_sensor(self, sensor_num):
        ...
        Ain_n = 6 # negative analog input (AIN) number name. it shall always be pin AIN 7 forced at REFP (positve reference input) nominaly at 2.5 V
        ...
        raw = self.fetch(Ain_n,[Ain_p]) # retreive the conversion data of sensor betwen the pin AIN7
        try:
            result =  self.vref_2*(1+raw[0]/((2**23)- 1)) # Coverter conversion in volt
        except:
    ...
    def Read_internal_temperature(self): # in this configuration Can read temperature up to 49 °C. If higher, the gain shall be changed
        ...
        self.wreg(2, [0x33]) # In the MUX1 register (2) ==> the ref voltage set on the internal reference and AIN input on the internal temeprature sensor
        # the set up induce by the ligne above forces the gain to 1 
        # get the conversion
        raw = self.receive()
        # Back to default settings
        self.wreg(2, [0x20]) # In the MUX1  register (2) ==> internal reference enabled, REFP0 and REFN0 reference inputs selected (0x20)
        # Computing the temperature
        try: # if reading sucesfull
            Vin = self.internal_Vref * raw / ((2**23)-1) # convert conversion in volt
            internal_temperature = 2469.135802 * Vin -  266.3580247 # convert volt value into temperature in °C. equaiton deduced form the datasheet p10
        except:
