...
## varible definition
position_low_pressure_sensor = 1
...
with open(name+'_data.csv', 'w', newline='') as data_log:
    ...
    data_writter.writerow(['Time_log_sec', 'voltage sensor (V)' ,'Pressure (bar)', 'RTD1_1 (*C) (panel)', 'RTD1_2 (*C) (PI)', 'ADS internal temp (*C)' ])
...
while True:
    ...
    RTD_1_1 = adc1.Read_RTD(2) # 2 reading are taken and averaged
    RTD_1_2 = adc1.Read_RTD(3) # 2 reading are taken and averaged
    ...
    with open(name+'_data.csv', 'a', newline='') as data_log:
        data_writter = csv.writer(data_log, 'excel') # generate object file writter    
        data_writter.writerow([t, low_pressure_volt, low_pressure, RTD_1_1[1], RTD_1_2[1], ADS_temp ])
    ...
    if ADS_temp != None:
        ...
        print ('No value')
    print (' ')
    print (str(low_pressure_volt)+ ' V, the voltage reading of the high_pressure sensor ')
