#Description:
 ...
 # list of attributs:
  ...
  # log            (list of str) log with all the status of the controller at all the updates
  ...
  # thresh_ex_high (float) in CELSIUS extreme high temperature threshold ==> PWM of heater set as PWM_ex_low. (0 % by default, heater turned off)
  # thresh_high    (float) in CELSIUS high temperature limit ==>  PWM of heater set as PWM_low (10 % by default)
  # thresh_low     (float) in CELSIUS low temperature limit ==> PWM of heater set as PWM_hight (90 % by default)
  # thersh_ex_low  (float) in CELSIUS extreme low temperature ==> PWM of heater set as PWM_ex_high (100 % by default)
  ...
  # energy         (float) in Watts, energy consumption of the heater at the moment of the update
 #
 # list of methods:
  ...
  # logging()            generate the logg textnsate for current source mismatch
 #
 # 
 #           PWM
 #            /\
...
class Bang_bang:
    ...
    def update (self, Temperature):
        ...
        ## if temperature is extreme low or high
        if Temperature > self.thres_ex_high or Temperature < self.thres_ex_low : ## if temperature is extreme low or high
            if Temperature < self.thres_ex_low:
                self.pigpio.set_PWM_dutycycle(self.PWM_pin, self.PWM_ex_high) ## Set GPIO PWM to extreme HIGH
                self.PWM_value = self.pigpio.get_PWM_dutycycle(self.PWM_pin) # Update the PWM value for report in the logs
                self.mem = True # update memory to True. the system in on the left side of hysteresis (high side of the hysteresis)
            else:
                self.pigpio.set_PWM_dutycycle(self.PWM_pin, self.PWM_ex_low)  ## Set GPIO PWM to extreme low
                self.PWM_value = self.pigpio.get_PWM_dutycycle(self.PWM_pin) # Update the PWM value for report in the logs
                self.mem = False # update memory to False. the system in on the right side of hysteresis (low side of the hysteresis)
            #self.logging()
            self.energy = (self.PWM_value * 0.01 * self.heater.voltage_in **2) /self.heater.resistor # Compute the energy consumption after the update: E = PWM * U^2 / R
            return

        ## arrives here if temperature is not extreme ==> T < thres_ex_high OR T > Thresh_ex_low)
        if self.mem:
